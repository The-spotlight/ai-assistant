...
export default function ArchivePanel({
  ...
  const filteredArchiveList = useMemo(() => {
    ...
    return archiveList.filter(c => 
      c.title?.toLowerCase().includes(query)
    );
  }, [archiveList, searchQuery]);

  const allSelected = filteredArchiveList.length > 0 && 
    filteredArchiveList.every(c => selectedIds.has(c.id));
  const someSelected = selectedIds.size > 0 && !allSelected && 
    filteredArchiveList.some(c => selectedIds.has(c.id));

  const toggleSelectAll = useCallback(() => {
    if (allSelected) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(filteredArchiveList.map(c => c.id)));
    }
  }, [allSelected, filteredArchiveList]);
      ...
      >
        ...
        <div className="px-4 py-3 flex items-center justify-between border-b border-black/[0.06] bg-[#fafafa]">
          <div className="flex items-center gap-2">
            <IconArchive className="h-4 w-4 text-[#737373]" />
            <span className="text-sm font-medium text-[#171717]">已存档</span>
        ...
        <div className="px-4 py-2 border-b border-black/[0.06] bg-[#fafafa]">
          <div className="flex items-start gap-2">
            <IconArchive className="h-4 w-4 text-[#737373] shrink-0 mt-0.5" />
            <div>
              <p className="text-[11px] font-medium text-[#525252]">存档的会话</p>
              <p className="text-[10px] text-[#a3a3a3]">可随时恢复到主列表，或彻底删除</p>
            </div>
          </div>
        </div>
          ...
          {filteredArchiveList.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12">
              {searchQuery.trim() ? (
                ...
                <>
                  <IconArchive className="h-12 w-12 text-[#d4d4d4] mb-3" />
                  <p className="text-sm font-medium text-[#737373] mb-1">存档为空</p>
                  <p className="text-xs text-[#a3a3a3]">存档的会话会显示在这里</p>
                </>
              )}
            </div>
          ) : (
            <div className="flex flex-col gap-1">
              {filteredArchiveList.map((c) => {
                    ...
                    >
                      ...
                      {showTime && (
                        <span className={`mt-1 block text-[11px] text-[#a3a3a3] ${
                          ...
                          存档于 {formatDate(c.archivedAt)} ({formatRelativeTime(c.archivedAt)})
                        </span>
                      )}
                    </button>
                    <button
