...
export default function EmojiPicker({ visible, onClose, onSelectEmoji }: EmojiPickerProps) {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(`/api/emojis?q=${encodeURIComponent(query.trim())}`);
      const data = await response.json();
      if (data.success && data.data) {
        ...
        {emojis.map((item, index) => {
          ...
          return (
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded text-lg transition-colors hover:bg-[#f0f0f0] dark:hover:bg-[#3d3d3d]"
              title={name}
            >
              {emoji}
            </button>
  ...
  const categoriesToShow = isSearching
    ? [{ id: 'search', name: '搜索结果', emojis: searchResults }]
    : [
  ...
  return (
    <div
      className="absolute bottom-full left-0 right-0 z-30 mb-2 overflow-hidden rounded-lg border border-[rgba(0,0,0,0.08)] bg-white dark:bg-[#262626] shadow-[0_4px_24px_rgba(0,0,0,0.12)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)]"
      role="dialog"
      aria-label="表情选择"
    >
      <div className="flex items-center justify-between border-b border-[rgba(0,0,0,0.08)] dark:border-white/10 bg-[#fafafa] dark:bg-[#171717] px-2.5 py-1.5">
        <h3 className="text-sm font-semibold text-[#171717] dark:text-white">表情</h3>
        <button
          type="button"
          onClick={onClose}
          className="flex h-6 w-6 items-center justify-center rounded text-[#808080] transition-colors hover:bg-[#ebebeb] dark:hover:bg-[#3d3d3d] hover:text-[#171717] dark:hover:text-white"
          aria-label="关闭"
        >
      ...
      <div className="px-2.5 py-1.5 border-b border-[rgba(0,0,0,0.08)] dark:border-white/10">
        <div className="relative">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[#a3a3a3]" />
          <input
            type="text"
            value={searchQuery}
            onChange={handleSearchChange}
            placeholder="搜索表情..."
            className="w-full h-7 pl-7 pr-2.5 text-xs border border-[rgba(0,0,0,0.08)] dark:border-white/10 rounded bg-white dark:bg-[#171717] text-[#171717] dark:text-white placeholder:text-[#a3a3a3] focus:outline-none focus:ring-1 focus:ring-[#171717] dark:focus:ring-white"
          />
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          categoriesToShow.map((category) => (
            <button
              ...
              className={`flex-shrink-0 px-2.5 py-1.5 text-xs font-medium transition-colors border-b-2 ${
      ...
      <div className="max-h-48 overflow-y-auto p-2">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center py-6 text-xs text-[#a3a3a3]">
            暂无最近使用的表情
          </div>
        ) : activeCategoryData?.emojis.length === 0 ? (
          <div className="flex items-center justify-center py-6 text-xs text-[#a3a3a3]">
            {isSearching ? '未找到匹配的表情' : '暂无表情数据'}
          </div>
        ) : (
      ...
      {!isSearching && recentEmojis.length > 0 && activeCategory === 'recent' && (
        <div className="border-t border-[rgba(0,0,0,0.08)] dark:border-white/10 px-2.5 py-1.5">
          <p className="text-[10px] text-[#737373] dark:text-[#a3a3a3]">点击表情插入，面板保持打开以便连续选择</p>
        </div>
      )}
    </div>
  );
